(() => {
    ...
    function rangeInit() {
        const computerSLider = document.querySelector("#computer-range .header-window__range");
        const serverSLider = document.querySelector("#servers-range .header-window__range");
        const timeSLider = document.querySelector("#time-range .header-window__range");
        if (computerSLider) {
            computerSLider.getAttribute("data-from");
            computerSLider.getAttribute("data-to");
            const startValue = document.querySelector("#computer-range ._start-value");
            initialize(computerSLider, {
                start: 9,
                connect: [ true, false ],
                step: 1,
                range: {
            ...
            computerSLider.noUiSlider.on("update", (function(values, handle) {
                startValue.value = parseInt(values[0]);
            }));
        }
        if (serverSLider) {
            serverSLider.getAttribute("data-from");
            serverSLider.getAttribute("data-to");
            const startValue = document.querySelector("#servers-range ._start-value");
            initialize(serverSLider, {
                start: 1,
                connect: [ true, false ],
                range: {
                    min: [ 0 ],
        ...
        if (timeSLider) {
            timeSLider.getAttribute("data-from");
            timeSLider.getAttribute("data-to");
            const startValue = document.querySelector("#time-range ._start-value");
            const endValue = document.querySelector("#time-range ._end-value");
            ...
            timeSLider.noUiSlider.on("update", (function(values, handle) {
                startValue.value = parseInt(values[0]);
                if (values[1] == "23.59") endValue.value = 24; else endValue.value = parseInt(values[1]);
            }));
        }
    }
    rangeInit();
    ...
    function initCarSpoller() {
        const cards = document.querySelectorAll(".direction__card");
        const openClass = "_open";
        function cardOpen(card) {
            if (!card.classList.contains(openClass)) card.classList.add(openClass);
        }
        function removeClass(cardsArr) {
            cardsArr.forEach((card => {
                card.classList.contains(openClass) ? card.classList.remove(openClass) : card;
            }));
        }
        document.addEventListener("click", (e => {
            if (e.target.closest(".direction__card") && !e.target.closest("._open")) {
                removeClass(cards);
                cardOpen(e.target.closest(".direction__card"));
            }
        }));
    }
